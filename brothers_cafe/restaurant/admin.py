...
class ShopSettingsAdmin(admin.ModelAdmin):
    list_display = ['shop_name', 'location', 'gstin', 'phone', 'default_discount_percent']
    fieldsets = (
        ('Shop Info', {'fields': ('shop_name', 'location', 'address', 'phone', 'email', 'gstin', 'logo')}),
        ('Payment', {'fields': ('upi_id', 'upi_qr_code')}),
        ('Billing', {'fields': ('default_discount_percent',)}),
    )
...
class CategoryAdmin(admin.ModelAdmin):
    list_display = ['icon_preview', 'name', 'order', 'is_active', 'item_count']
...
class MenuItemAdmin(admin.ModelAdmin):
    list_display = ['image_preview', 'name', 'category', 'price', 'item_type', 'is_available', 'is_featured', 'order']
    ...
    def image_preview(self, obj):
        if obj.image:
            try:
                return format_html(
                    '<img src="{url}" style="height:54px;width:54px;object-fit:cover;border-radius:10px;box-shadow:0 2px 6px rgba(0,0,0,.18)"/>',
                    url=obj.image.url
                )
            except Exception:
                pass
        return format_html('<span style="font-size:22px">🍽️</span>')
    ...
    def image_preview_large(self, obj):
        if obj.image:
            try:
                return format_html(
                    '<img src="{}" style="max-height:200px;max-width:300px;object-fit:cover;border-radius:12px;box-shadow:0 4px 12px rgba(0,0,0,.2)"/>',
                    obj.image.url
                )
            except Exception:
                pass
...
class ComboAdmin(admin.ModelAdmin):
    ...
    def combo_image_preview(self, obj):
        if obj.image:
            try:
                return format_html(
                    '<img src="{}" style="height:54px;width:54px;object-fit:cover;border-radius:10px;box-shadow:0 2px 6px rgba(0,0,0,.18)"/>',
                    obj.image.url
                )
            except Exception:
                pass
        return format_html('<span style="font-size:28px;line-height:1">{}</span>', obj.icon or '🎁')
